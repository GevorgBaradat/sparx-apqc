//ea_diagram_to_svg 2026-07-05-v38
// VERSION: 2026-07-05-v38 (fisheye lens: Furnas 1986; radial focus+context; D/R controls)
/**
 * ea_diagram_to_svg.js  (v38)
 * ==============================
...
function generateSVG(diagID, diagName, outDir, depth) {
    ...
    /* СВЯЗИ — всегда <path> с data-orig-d (нужно для fisheye в v38) */
    ...
    for(var ci2=0;ci2<connectors.length;ci2++){
        ...
        L.push('<path d="'+d+'" '+attrs+' data-orig-d="'+d+'"/>');
    }
    L.push('');
    ...
    L.push('      <div style="margin-top:8px;border-top:1px solid #e0e4ea;padding-top:7px;">');
    L.push('        <div style="font-weight:700;color:#69738C;margin-bottom:5px;letter-spacing:0.4px;">FISHEYE</div>');
    L.push('        <div style="display:flex;align-items:center;gap:4px;margin:3px 0;">');
    L.push('          <button data-act="fish" id="ea-fish-btn" style="flex:1;padding:5px 6px;border-radius:5px;border:1px solid #c8d0dc;cursor:pointer;font-size:11px;background:#f0f4f8;color:#333;">&#128065; выкл</button>');
    L.push('        </div>');
    L.push('        <div style="display:flex;align-items:center;gap:6px;margin:3px 0;"><span style="width:78px;">Сила D</span><button data-act="fish-d-" style="width:22px;height:20px;cursor:pointer;">&#8722;</button><span id="ea-fd-v" style="width:40px;text-align:center;">5</span><button data-act="fish-d+" style="width:22px;height:20px;cursor:pointer;">+</button></div>');
    L.push('        <div style="display:flex;align-items:center;gap:6px;margin:3px 0;"><span style="width:78px;">Радиус R</span><button data-act="fish-r-" style="width:22px;height:20px;cursor:pointer;">&#8722;</button><span id="ea-fr-v" style="width:40px;text-align:center;">150</span><button data-act="fish-r+" style="width:22px;height:20px;cursor:pointer;">+</button></div>');
    L.push('      </div>');
    ...
    L.push("    else if(act==='fish'){fishOn=!fishOn;if(!fishOn)resetFish();setFishUI();}");
    L.push("    else if(act==='fish-d+'){FD=Math.min(15,FD+1);setFishUI();}");
    L.push("    else if(act==='fish-d-'){FD=Math.max(1,FD-1);setFishUI();}");
    L.push("    else if(act==='fish-r+'){FR=Math.min(400,FR+25);setFishUI();}");
    L.push("    else if(act==='fish-r-'){FR=Math.max(50,FR-25);setFishUI();}");
    ...
    L.push("  /* ── FISHEYE (v38) ─────────────────────────────────────────────────────");
    L.push("     Алгоритм Фёрнаса (Xerox PARC, 1986): точки внутри радиуса FR");
    L.push("     притягиваются к фокусу с коэффициентом (FD+1)*FR/(FD*d+FR).");
    L.push("     Элементы трансформируются через translate+scale на группе <g>;");
    L.push("     пути коннекторов — покоординатно через data-orig-d.          */");
    L.push("  var fishOn=false, FD=5, FR=150;");
    L.push("  function fishPt(px,py,fx,fy){");
    L.push("    var dx=px-fx,dy=py-fy,d=Math.sqrt(dx*dx+dy*dy);");
    L.push("    if(d<0.01)return{x:fx,y:fy,s:FD+1};");
    L.push("    if(d>=FR) return{x:px,y:py,s:1};");
    L.push("    var sc=(FD+1)*FR/(FD*d+FR);");
    L.push("    return{x:fx+dx*sc,y:fy+dy*sc,s:sc};}");
    L.push("  function fishPath(origD,fx,fy){");
    L.push("    return origD.replace(/([ML])\\s*(-?[\\d.]+),(-?[\\d.]+)/g,function(_,cmd,xs,ys){");
    L.push("      var f=fishPt(parseFloat(xs),parseFloat(ys),fx,fy);");
    L.push("      return cmd+' '+f.x.toFixed(1)+','+f.y.toFixed(1);});");
    L.push("  }");
    L.push("  function applyFish(fx,fy){");
    L.push("    var els=document.querySelectorAll('.ea-el');");
    L.push("    for(var i=0;i<els.length;i++){");
    L.push("      var g=els[i],id=g.getAttribute('data-id'),r=EA_RECTS[id];");
    L.push("      if(!r)continue;");
    L.push("      var ocx=r.x+r.w/2,ocy=r.y+r.h/2;");
    L.push("      var f=fishPt(ocx,ocy,fx,fy);");
    L.push("      g.setAttribute('transform','translate('+f.x.toFixed(1)+','+f.y.toFixed(1)+') scale('+f.s.toFixed(3)+') translate('+(-ocx).toFixed(1)+','+(-ocy).toFixed(1)+')');}");
    L.push("    var paths=document.querySelectorAll('[data-orig-d]');");
    L.push("    for(var j=0;j<paths.length;j++){var p=paths[j];p.setAttribute('d',fishPath(p.getAttribute('data-orig-d'),fx,fy));}}");
    L.push("  function resetFish(){");
    L.push("    var els=document.querySelectorAll('.ea-el');");
    L.push("    for(var i=0;i<els.length;i++)els[i].removeAttribute('transform');");
    L.push("    var paths=document.querySelectorAll('[data-orig-d]');");
    L.push("    for(var j=0;j<paths.length;j++)paths[j].setAttribute('d',paths[j].getAttribute('data-orig-d'));}");
    L.push("  function setFishUI(){");
    L.push("    var btn=document.getElementById('ea-fish-btn');");
    L.push("    if(btn){btn.textContent=fishOn?'\\uD83D\\uDC1F вкл':'\\uD83D\\uDC41 выкл';");
    L.push("      btn.style.background=fishOn?'#1a2e5a':'#f0f4f8';");
    L.push("      btn.style.color=fishOn?'#fff':'#333';}");
    L.push("    var fdv=document.getElementById('ea-fd-v');if(fdv)fdv.textContent=FD;");
    L.push("    var frv=document.getElementById('ea-fr-v');if(frv)frv.textContent=FR;");
    L.push("    scene.style.cursor=fishOn?'crosshair':'default';}");
    L.push("  scene.addEventListener('mousemove',function(e){");
    L.push("    if(!fishOn)return;");
    L.push("    var p=toSvg(e);");
    L.push("    var fx=p.x/diagScale,fy=p.y/diagScale;");
    L.push("    applyFish(fx,fy);});");
    L.push("  scene.addEventListener('mouseleave',function(){if(fishOn)resetFish();});");
